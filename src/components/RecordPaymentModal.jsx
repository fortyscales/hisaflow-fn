...
const RecordPaymentModal = ({ visible, creditSale, onSave, onClose, paymentAccounts = [] }) => {
  const { t } = useLanguage();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await onSave(creditSale.id, amt, paymentMethod, account);
      if (!result.success) {
  ...
  return (
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <label style={styles.label}>{t("receivedIntoAccountLabel")}</label>
